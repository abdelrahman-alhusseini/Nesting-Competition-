...
-- Seed/update every existing competition so the currently active competition is
-- changed immediately when this SQL is run.
...
cross join (
  values
    ...
    -- Restoration: Move Another Player Down 2 = 12%
    ('restoration', 'move_self_up_2', 44.0000::numeric),
    ('restoration', 'move_player_2',  12.0000::numeric),
    ('restoration', 'shield',         44.0000::numeric),

    -- Remodeling Cross-Sell: Move Another Player Down 2 = 80%
    -- Remaining 20% is split across the existing strongest remodeling cards.
...
begin
  ...
  if random() < v_special_chance then
    ...
    -- Weighted special-card selection. The configured weights are normalized at
    -- draw time, so they do not have to total exactly 100 in future edits.
    with weighted as (
      select
        scw.card_code,
        scw.weight,
    ...
    -- Legacy fallback protects future competitions if their weight rows have not
    -- been seeded yet.
...
-- Verification query: every booking-type pool should total 100.
select
