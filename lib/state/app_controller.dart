...
class AppController extends ChangeNotifier {
  ...
  // Agent-facing feed is intentionally limited to actual card/game actions.
  // Booking approvals/submissions, manual grants, and admin score adjustments
  // are not shown here. Player names are resolved from profiles.
  List<String> get agentGameActivity => _activityRecords
      .where((ActivityRecord item) => item.isAgentGameActivity)
      .map((ActivityRecord item) => item.playerMessage)
      .toList(growable: false);
  int get bookingCount => _role == AppRole.admin
      ? _adminStats.totalBookings
      : _myBookings.length;
  List<BookingRecord> get myBookings => List<BookingRecord>.unmodifiable(_myBookings);
  ...
  Future<void> refreshAll() async {
    ...
    try {
      ...
      final List<LeaderboardEntry> leaderboard = await repository.getLeaderboard();
      ...
      if (_role == AppRole.agent) {
        ...
        final List<dynamic> results = await Future.wait<dynamic>(<Future<dynamic>>[
          ...
          repository.getAdminPendingDrawCount(),
        ]);
        ...
        _adminPendingDrawCount = results[5] as int;
      }
      _lastError = null;
    } catch (error) {
      _lastError = _friendlyError(error);
  ...
  Future<String?> updateUsername(UserProfile user, String username) async {
    final SupabaseRepository? repository = _repository;
    if (repository == null) return 'Supabase is not configured.';
    ...
    _setBusy(true);
    try {
      await repository.updateUserCredentials(userId: user.id, username: value);
      await refreshAll();
      return null;
    } catch (error) {
      return _friendlyError(error);
  ...
  Future<String?> updateUserPassword(UserProfile user, String password) async {
    final SupabaseRepository? repository = _repository;
    if (repository == null) return 'Supabase is not configured.';
    if (password.length < 6) return 'Password must contain at least 6 characters.';
    _setBusy(true);
    try {
      await repository.updateUserCredentials(userId: user.id, password: password);
      return null;
    } catch (error) {
      return _friendlyError(error);
    } finally {
  ...
  void drawUnlimitedAdminTestCard() {
    _prepareUnlimitedAdminTestCard();
    _adminPage = AdminPage.cardReveal;
    notifyListeners();
  }

  /// Generates a non-destructive admin test card while staying on the
  /// Manual Draw page. This can be used without limits and never writes a
  /// booking, pending draw, score change, or card draw to Supabase.
  void drawUnlimitedAdminTestCardInline() {
    _prepareUnlimitedAdminTestCard();
    _adminPage = AdminPage.testDraw;
    notifyListeners();
  }

  void _prepareUnlimitedAdminTestCard() {
  ...
  Future<void> drawAgentCard() async {
    ...
    _setBusy(true);
    try {
      final DrawResult result = await repository.drawCard(_pendingDrawRecords.first.id);
  ...
  Future<void> gambleEvenCard({required bool adminTest}) async {
    ...
    if (!_gamblePending || currentOutcome == null) return;
    if (adminTest) {
      final double winChance = _gameSettings?.gamblePlusFourChance ?? 0.50;
      final bool won = _random.nextDouble() < winChance;
      _currentOutcome = CardOutcome(
        title: won ? '+4 Points' : '-6 Points',
        description: won
            ? 'The gamble paid off.'
            : 'The gamble did not go your way.',
  ...
  Future<void> _resolveEvenCard({required bool gamble}) async {
    ...
    try {
      ...
      _currentOutcome = result.outcome;
      gamble && result.outcome.isNegative ? _audio.negative() : _audio.positive();
      await refreshAll();
    } catch (error) {
      _lastError = _friendlyError(error);
    } finally {
  ...
  Future<String?> saveCurrentSpecialCard({required bool replaceExisting}) async {
    final SupabaseRepository? repository = _repository;
    final String? cardDrawId = _currentCardDrawId;
  ...
  Future<String?> saveGameSettings(GameSettings settings) async {
    final SupabaseRepository? repository = _repository;
    if (repository == null) return 'Supabase is not configured.';
    _setBusy(true);
    try {
      await repository.updateGameSettings(settings);
      await refreshAll();
      return null;
    } catch (error) {
      return _friendlyError(error);
