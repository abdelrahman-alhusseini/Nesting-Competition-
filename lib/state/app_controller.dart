...
class AppController extends ChangeNotifier {
  ...
  Future<String?> login({
    ...
    _setBusy(true);
    try {
      await _repository!.signIn(username: cleanUsername, password: password);
      final UserProfile profile = await _repository!.getCurrentProfile();
      if (!profile.isActive) {
        await _repository!.signOut();
  ...
  Future<void> refreshAll() async {
    if (_repository == null || !_loggedIn || _refreshing) return;
    _refreshing = true;
    try {
      final UserProfile freshProfile = await _repository!.getCurrentProfile();
      final List<ActivityRecord> activity = await _repository!.getActivity();
      final List<LeaderboardEntry> leaderboard = await _repository!.getLeaderboard();
      ...
      if (_role == AppRole.agent) {
        final List<dynamic> results = await Future.wait<dynamic>(<Future<dynamic>>[
          ...
          _repository!.getMyPendingDraws(),
          _repository!.getMySavedSpecialCard(),
        ]);
  ...
  Future<String?> submitBooking(String link, BookingType bookingType) async {
    if (_repository == null) return 'Supabase is not configured.';
    _setBusy(true);
    try {
      await _repository!.submitBooking(jobUrl: link, bookingType: bookingType);
      await refreshAll();
      return null;
    } catch (error) {
      return _friendlyError(error);
  ...
  Future<String?> setUserActive(UserProfile user, bool isActive) async {
    ...
    _setBusy(true);
    try {
      await _repository!.setUserActive(userId: user.id, isActive: isActive);
      await refreshAll();
      return null;
    } catch (error) {
      return _friendlyError(error);
  ...
  Future<void> drawAgentCard() async {
    if (_repository == null || _pendingDrawRecords.isEmpty || _busy) return;
    _setBusy(true);
    try {
      final DrawResult result = await _repository!.drawCard(_pendingDrawRecords.first.id);
  ...
  Future<void> keepEvenCardPoint({required bool adminTest}) async {
    if (!_gamblePending || _currentOutcome == null) return;
    if (adminTest) {
  ...
  Future<void> _resolveEvenCard({required bool gamble}) async {
    if (_repository == null || _currentCardDrawId == null) return;
    _setBusy(true);
    try {
      final DrawResult result = await _repository!.resolveEvenCard(
        cardDrawId: _currentCardDrawId!,
        gamble: gamble,
      );
      _currentOutcome = result.outcome;
  ...
  Future<String?> saveCurrentSpecialCard({required bool replaceExisting}) async {
    if (_repository == null || _currentCardDrawId == null) {
      return 'No special card is waiting to be saved.';
    }
    _setBusy(true);
    try {
      await _repository!.saveSpecialCard(
        cardDrawId: _currentCardDrawId!,
  ...
  Future<String?> grantAgentDraw({
    ...
    _setBusy(true);
    try {
      await _repository!.manualGrantDraw(
        agentId: agentId,
